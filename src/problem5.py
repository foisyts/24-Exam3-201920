...
def problem5(seq_of_seq):
    ...
    for k in range(len(seq_of_seq)):
        for j in range(len(seq_of_seq[k])):
            first_largest = seq_of_seq[k][0]
            if j == 0:
                current_largest = first_largest
            if j != 0:
                if current_largest <= seq_of_seq[k][j]:
                    current_largest = seq_of_seq[k][j]
        new_list = new_list + [current_largest]
